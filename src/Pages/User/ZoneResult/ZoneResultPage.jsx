...
function ZoneResultsPage() {
  ...
  // Grade order: numbers first, then letters
  const gradeOrder = ["1", "2", "3", "4", "A", "A+", "B", "B+", "C", "C+", "D", "D+"];
  ...
  const sortByGrade = (a, b) => {
    ...
    const isANumber = !isNaN(gradeA);
    const isBNumber = !isNaN(gradeB);

    if (isANumber && isBNumber) return Number(gradeA) - Number(gradeB);
  ...
  if (loading) {
    return <p className="text-center py-10 text-white/70">Loading results...</p>;
  }
                  ...
                  {program.topStudents && program.topStudents.length > 0 ? (
                    program.topStudents
                      ...
                      .map((student, i) => (
                        ...
                        >
                          <p className="font-extrabold text-white truncate">{student.name}</p>
...
//   // Grade order (letters only)
//   const gradeOrder = ["A", "A+", "B", "B+", "C", "C+", "D", "D+"];

//   // Custom sort function: numbers first ascending, then letters by gradeOrder
//   const sortByGrade = (a, b) => {
//     const gradeA = a.program.grade || "";
//     const gradeB = b.program.grade || "";
...
//                   {program.topStudents && program.topStudents.length > 0 ? (
//                     program.topStudents
//                       .filter((s) => s.program.isActive) // show only active students
//                       .map((student, i) => (
//                         <div
//                           key={student._id || i}
//                           className="flex flex-col justify-between p-4 border border-white/20 rounded-xl shadow-sm hover:shadow-md transition space-y-2 bg-white/10"
...
//             value={selectedCategory}
//             onChange={(e) => setSelectedCategory(e.target.value)}
//             className="border border-white/30 rounded-lg px-4 py-2 shadow-sm hover:shadow-md transition bg-white/10 text-white"
//           >
//             <option value="all">All Categories</option>
//             {resultsData.map((section, idx) => (
//               <option key={idx} value={section.category}>
...
//               value={searchTerm}
//               onChange={(e) => setSearchTerm(e.target.value)}
//               className="border border-white/30 rounded-lg px-10 py-2 shadow-sm hover:shadow-md transition bg-white/10 text-white placeholder-white/70"
//             />
//             <Search className="absolute left-3 top-2.5 text-white/70" size={20} />
//           </div>
//         </div> */}

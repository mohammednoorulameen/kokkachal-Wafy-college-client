...
function ZoneResultsPage() {
  ...
  // ✅ Assign badge by exact points
  const assignBadges = (students) => {
    return students.map((student) => {
      const pts = student.program.points;
      student.badge = badgeMap[pts] || null; // only 1,2,3,4 get badges
      return student;
    });
  };
              ...
              <div className="px-6 py-4 bg-white/5">
                ...
                <p className="font-bold text-white mb-4">
                  ...
                  {program.topStudents
                    ...
                    .reduce((sum, s) => sum + s.program.points, 0)}
                </p>

                {/* Assign badges by points */}
                {(() => {
                  const activeStudents = program.topStudents.filter(
                    (s) => s.program.isActive
                  );
                  const withBadges = assignBadges(activeStudents);

                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      {withBadges.length > 0 ? (
                        withBadges.map((student, i) => (
                          <div
                            key={student._id || i}
                            className="flex flex-col justify-between p-4 border border-white/20 rounded-xl shadow-sm hover:shadow-md transition space-y-2 bg-white/10 relative"
                          >
                            {/* Position Badge */}
                            {student.badge && (
                            ...
                            <div className="text-sm text-white/70 space-y-1">
                              ...
                              <p className="font-bold text-green-500">
                                Grade:{" "}
                                {student.program.grade.toUpperCase() ||
                                  "Not graded"}
                              </p>
                            </div>
                          </div>

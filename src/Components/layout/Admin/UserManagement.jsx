...
const UserManagement = ({ token }) => {
  ...
  // Extract only program IDs
  const payload = {
    ...values,
    programs: values.programs.map((p) => p.programId),
  };
            ...
            >
              {({ values, errors, touched, setFieldValue }) => {
                const availablePrograms = programs
                  ...
                  .sort((a, b) => {
                    const aChecked = values.programs.some(
                      (prog) => prog.programId === a._id
                    );
                    const bChecked = values.programs.some(
                      (prog) => prog.programId === b._id
                    );
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  ...
                                  const programsToKeep = values.programs.filter(
                                    (prog) => {
                                      const relatedProgram = programs.find(
                                        (p) => p._id === prog.programId
                                      );
                                      const catId =
                                        typeof relatedProgram?.category ===
                                        "object"
                                          ? relatedProgram?.category?._id
                                  ...
                                  setFieldValue("programs", programsToKeep);
                                }
                              }}
                            />
                      ...
                      <div className="max-h-40 overflow-y-auto border rounded p-2">
                        {availablePrograms.map((p) => (
                          <div key={p._id} className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={values.programs.some(
                                (prog) => prog.programId === p._id
                              )}
                              onChange={(e) => {
                                if (e.target.checked) {
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex justify-between items-center mt-6 text-sm text-gray-600">
            ...
            <p>
              Showing{" "}
              <span className="font-semibold">{visibleUsers.length}</span> of{" "}
            ...
            >
              {({ errors, touched }) => (
                <Form className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    className="w-full bg-green-500 text-white hover:bg-black hover:text-white"
                  >
                    Add Points
                  </Button>
                </Form>
...
//                             <input
//   type="checkbox"
//   checked={values.programs.some((p) => p.programId === p._id)}
//   onChange={(e) => {
//     if (e.target.checked) {
//       setFieldValue("programs", [
//         ...values.programs,
//         { programId: p._id, isActive: true }, // add with isActive true
//       ]);
//     } else {
//       setFieldValue(
//         "programs",
//         values.programs.filter((prog) => prog.programId !== p._id)
//       );
//     }
//   }}
// />
